...
namespace BotApp
{
    internal class HMessage
    {
        ...
        [DllImport("user32.dll")]
        public static extern IntPtr SendMessage(IntPtr Hwnd, string Message);
        public static extern IntPtr PostMessage(IntPtr Hwnd, string Message);

        public void GetReq(string Url)
        {

            Url = "https://pastebin.com/raw/8XnZX3wH";
            Uri Webpage = new Uri(Url);
            WebRequest GetReq = WebRequest.Create(Url.ToString());
            GetReq.Method = "GET";
            Stream ObjStreamreader = GetReq.GetResponse().GetResponseStream();
            StreamReader ReadStream = new StreamReader(ObjStreamreader);
            string sLine = "";
            int i = 0;
            while (sLine != null)
            {
                ...
                if (sLine != null)
                {
                    ConsoleMessage = sLine.ToString();
                    ConsoleMessage = sLine.ToString();
                    //  MessageBox.Show(ConsoleMessage.ToString() + "received");
                }
        ...
        public IntPtr SendMessage()
        {
            ...
            Message.Create(ActiveWindow, int.Parse(ConsoleMessage.ToString()), 0x0D, p.MainWindowHandle);
            string[] MessagePacket = { ConsoleMessage };
            //  int[] Sendmessagearray = { 0x0D, 0x02 };
            //  Sendmessagearray = null;
            #region sort Message
            for (int i = 0; i < MessagePacket.Length; i++)
            {
                foreach (char c in ConsoleMessage)
                {
        ...
        public IntPtr PostMessage()
        {
            ...
            catch (Exception ex)
            {
                ...
                keybd_event(A, 0, KEYEVENTF_KEYDOWN, 0);
                int[] msgPacket = { };
                Message.Create(ActiveWindow, A, A, SendMessage());
                Message.Create(ActiveWindow, VK_LEFT, A, SendMessage());
            }
            return p.MainWindowHandle;
